...
def is_profile_or_blog_index_page(title: str, text: str) -> bool:
    title_clean = " ".join(str(title or "").split()).lower()
    text_head = " ".join(str(text or "").split()).lower()[:1400]
    if "새로운걸 공부하고 기록하는 것을 좋아합니다" in title_clean:
        return True
    if "backend engineer" in text_head:
        return True
    if "devlog" in text_head and "기술 면접 대비 cs전공 핵심요약집" not in text_head and "프로세스" not in text_head:
        return True
    return False
    ...
    **kwargs,
    ...
    for idx, item in enumerate(selected_toc_links, start=2):
        ...
        try:
            ...
            if is_profile_or_blog_index_page(getattr(page, "title", "") or "", getattr(page, "text", "") or ""):
                skipped += 1
                trace.event(
                    "oopy_toc_page_skipped_profile_or_blog_index",
                    order=idx - 1,
    ...
    relevant_toc_total = max(0, toc_total - skipped)
    toc_missing = max(0, relevant_toc_total - collected)
    toc_coverage = collected / relevant_toc_total if relevant_toc_total else 0.0
    ...
    graph["quality"]["toc_relevant_candidates"] = relevant_toc_total
    ...
    graph["quality"]["toc_skipped_noise"] = skipped
